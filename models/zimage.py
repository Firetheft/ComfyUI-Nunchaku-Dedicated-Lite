...
def get_dtype_and_device(linear: nn.Linear):
    if linear.weight is not None:
        return linear.weight.dtype, linear.weight.device
    dtype = getattr(linear, "weight_comfy_model_dtype", torch.bfloat16)
    factory_kwargs = getattr(linear, "factory_kwargs", {})
    device = factory_kwargs.get("device", torch.device("cpu"))
    return dtype, device

def create_svdq_linear(linear: nn.Linear, **kwargs):
    dtype, device = get_dtype_and_device(linear)
...
def fuse_to_svdquant_linear(comfy_linear1: nn.Linear, comfy_linear2: nn.Linear, **kwargs) -> SVDQW4A4Linear:
    ...
    dtype, device = get_dtype_and_device(comfy_linear1)
    return SVDQW4A4Linear(
...
class ComfyNunchakuZImageAttention(JointAttention):
    ...
    def __init__(self, orig_attn: JointAttention, **kwargs):
        ...
        self.qkv = create_svdq_linear(orig_attn.qkv, **kwargs)
        self.out = create_svdq_linear(orig_attn.out, **kwargs)
...
class ComfyNunchakuZImageFeedForward(nn.Module):
    ...
    def __init__(self, orig_ff: FeedForward, **kwargs):
        ...
        self.w2 = create_svdq_linear(orig_ff.w2, **kwargs)
